    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/bootstrap":
            ...
            functions = conn.execute(
                """
                SELECT DISTINCT f.id, f.code, f.name, f.description AS definition
                FROM functions f
            ...
            return self._json({"me": me, "functions": [dict(x) for x in functions]})
        ...
        if parsed.path == "/api/controls":
            ...
            # Filter option payload for selected function (UX filter controls)
            filter_where = ["u.id=?"]
            filter_vals = [user["user_id"]]
            if function_id:
                filter_where.append("f.id=?")
                filter_vals.append(function_id)
